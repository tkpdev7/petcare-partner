...
export default function AddServiceScreen() {
  ...
  const handleSave = async (values: any, { setSubmitting }: any) => {
    try {
      ...
      if (!response.success) {
        Alert.alert('Error', response.error || `Failed to {u20b9}{isEditMode ? 'update' : 'add'} service`);
        return;
      }

      Alert.alert('Success', `Service {u20b9}{isEditMode ? 'updated' : 'added'} successfully`, [
        { text: 'OK', onPress: () => router.back() }
      ]);
    } catch (error) {
      console.error(`{u20b9}{isEditMode ? 'Update' : 'Create'} service error:`, error);
      Alert.alert('Error', `Failed to {u20b9}{isEditMode ? 'update' : 'add'} service`);
    } finally {
      setSubmitting(false);
    }
  };
