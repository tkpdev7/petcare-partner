...
export default function ManageSlotsScreen() {
  ...
  const loadActiveStatus = async () => {
    try {
      ...
      console.error('Load active status error:', error);
    } finally {
  ...
  const handleCreateSlot = async () => {
    ...
    try {
      ...
      const endTime = new Date(startTime.getTime() + slotDuration * 60000); // Add duration in milliseconds
          ...
          <View style={styles.toggleContainer}>
            <View style={styles.toggleLabelContainer}>
              ...
              <Text style={styles.toggleSubLabel}>
                {isActiveOnline ? 'Customers can book your slots' : 'Offline - No bookings allowed'}
              </Text>
            </View>
            <Switch
